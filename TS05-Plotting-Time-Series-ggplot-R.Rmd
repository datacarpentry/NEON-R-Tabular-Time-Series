...
15-min data. We can add labels to the x- and y-axes using `xlab=` and `ylab=`.
...
We get a warning message reminding us that there are two missing values in the 
...
we code `na.rm=TRUE` within the geom_point() function.  This will prevent the
warning messages from showing.

Alternatively, specifications that apply to the overall figure are simply added
within the `ggplot()` function by adding them (with `+`) after the last
...
For more information on color, including a color blind friedly color pallete, 
read the  <a href="http://www.cookbook-r.com/Graphs/Colors_(ggplot2)/" target="_blank">
ggplot2 color information</a> from Winston Chang's Cookbook for R site based on 
the R Graphics Cookbook. 

##Figures with Lines
Often when plotting data from a time series, representing the data with a line 
makes sense.  Let's try it with out daily air temperature data. 
```{r ggplot-geom_lines}
...
is correct for the data at hand.  {: .notice} 
...
Our data, three years of a well defined annual cycle, should be appropriate for
...
As our data set has 1095 observations the gam method was used with a specified 
formula.  An algarithm in this package determined the formula for this
line, if we had figured out a GAM for the air temperature data we could have 
input our own formula.  
...
#Challenge: Plot Montly Air Temperature
