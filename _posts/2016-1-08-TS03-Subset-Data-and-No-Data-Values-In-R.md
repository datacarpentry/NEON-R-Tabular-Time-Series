...
to a `POSIXct` class as covered in the lesson: [Dealing With Dates & Times in R - as.Date, POSIXct, POSIXlt]({{ site.baseurl }}/R/Time-Series-Metadata/). 
...
which designate `NoData`. Blanks are particularly problematic because we can't
be certain if a data value is purposefully missing (not measured that day or a
bad measurement) or if someone unintentionally deleted it.
...
How many `NoData` values are in the precipiation (`prec`) and PAR (`parr`)
columns of our data?
...
are `NA` values in the data, unless we explicetly tell it to ignore them. 
