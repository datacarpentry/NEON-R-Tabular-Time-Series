...
##About
In this lesson, we will use the `groupby`, `summarize`and `mutate` functions in the 
`dplyr` package to efficiently manipulate atmospheric data collected at the NEON
Harvard Forest Field Site. We will use of pipes to efficiently perform multiple
...
Download NEON Atmospheric Teaching Data Subset</a>
...
performed data "munging". It uses the `data.frame` object as both and input and 
an ouput.
...
`AtmosData/HARV/Met_HARV_15min_2009_2011.csv`. Be sure to convert the 
...
qplot(x=datetime, y=airt, 
           ...
           xlab="Date", ylab= "Air Temperature, Celcius")

qplot(x=datetime, y=prec, 
           data=harMet15.09.11, na.rm=TRUE,
           main=("Precipitation \n NEON Harvard Forest Field Site"),
           xlab="Date", ylab= "Daily Total Precip., mm")

qplot(x=datetime, y=parr, 
           data=harMet15.09.11, na.rm=TRUE,
           main=("Air Temp \n NEON Harvard Forest Field Site"),
...
or by year) may be easier to visually interpret and / or to compare with other data. 
...
Let's start by extracting a yearly air temperature value for our Harvard Forest
...
```{r group-by-dplyr }
    #Within the harMet15.09.11 data
har.grp.year <- group_by(harMet15.09.11, #object containing the data
         year) #column name -  that we wish to group by

#view class of the object
class(har.grp.year)
```

Note that the `group_by` function creates a "grouped" object. We can then use this
...
function. We can then use the `summarize` function to quickly calculate the mean
...
summarize(har.grp.year, 
          mean(airt)) #calculate the annual mean of airt

```

Uh oh. Why did this return a `NA` value for years 2009 and 2010? Let's check our
data for `noData` values.

```{r check-data }
#are there noData values?
sum(is.na(har.grp.year$airt))

#where are the no data values
#just view the first 6 columns of data
har.grp.year[is.na(har.grp.year$airt),1:6]
...
`har.grp.year`. We can combine these steps using `pipes` in the `dplyr` package.
...
4. We can then add an additional function to our pipe, we can tell `R` to `tally`
or count the number of measurements per year.
...
Using the piping syntax that we just learned, create a data.frame called jday.avg
...
to designate a pipe. Pipes are ocassionally sometimes referred to as chains. 
{: .notice }
...
However this time, we can group by two variables: year and jd as follows:
...
harMet15.09.11 %>%      #Within the harMet15.09.11 data
  group_by(year,jd) %>%      #group the data by the Year and Julian day
  tally()               #and tally (count) how many observations per julian day
```
...
dataset we could use the `yday()` function <a href="http://www.inside-r.org/packages/cran/lubridate/docs/yday"/>
...
Day. Calculate total `prec` for each Julian Day over the 3 years - name your
data.frame `total.prec`. Then create 
a plot of Daily Total Precipitation for 2009-2011. Make sure your plot has a title
and be sure to label the x and y axes appropriately.

HINT: You can use `colour="total.prec$year"` to color points by the year values
stored in your data.frame. 
DOUBLE HINT: You can also specify `as.factor(total.prec$year)` to
force year to be a categorical rather than continuous variable.
</div>
...
to a data.frame. For instance, in this lesson, we added the year independently
at the very beginning of the lesson. However, we can also add it as a part of our 
dplyr pipe.
...
In the challenge above, we tried to plot precipitation data using qplot. However,
this plot was a challenge given we were missing a continuous date variable
to use on the x axis (the year and date for each summary value). 

We can add the a value from hte date-time column to our data frame by adding and
...
So our new summarize statement in our pipe will look like this:
...
1. Plot daily Air Temperature from 2009-2011 as we did in the previous challenge.
However this time, use the new syntax that you learned (mutate and summarize to add a datetime
column to your output data.frame).
