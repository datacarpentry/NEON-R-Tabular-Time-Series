---
layout: post
title: "Lesson 06: Plotting with Facets and Time Series with NDVI data"
date:   2015-10-20
...
Forest. If you do not have these data sets as `R` objects, created in
...
Meterological data.  We can simply do this by in putting both variables into the
...
that `season` is now included in the data. 
...
"summer", "winter".  As no level was assigned as data class is character `R`
...
harMetDaily.09.11$season<- factor(harMetDaily.09.11$season, level=c("spring",                                                   "summer","fall","winter"))
...
##Plot Two Variables Across A Time Series. 
``` {r plot-bar-and-line}
