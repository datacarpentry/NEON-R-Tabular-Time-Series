---
layout: post
title: "Lesson 04: Manipulating and Subsett Time Series Data with `dplyr`"
date:   2015-10-21
...
`dplyr`functions including `groupby`, `summarize`and `mutate`. The use of pipes
...
in dataframes.  `dplyr` is a powerful package that has capabilities beyond the 
...
##Load the Data
To complete this lesson you will need the 15-minute micro-meteorology data for
...
please load it from the .csv file in the downloaded data.  
...
#If not already available from Lessons 03:
...
15-minute intervals that we have in our data.  However, summarizing the data at
...
all `dplyr` functions is the same: the first argument is the target data frame, 
the subsequent arguments dictate what to do with that data frame and the output 
is a new data frame. Once the data frame is specified, column names can be 
directly referred to.  

##Use of Pipes
Pipes (ocassionally called chains) are an important tool in `dplyr` that allow 
...
Bonus: Older `dplyr` coded pipes as %.% and you may still see that syntax in
some older StackOverflow answers and other tutorials. {: .notice2}
...
`mutate()` is used to add new data to a dataframe. These are often new data that
are created from a calculation or
manipulation of existing data. If you are familiar with the `transform()` base R
function the usage is similar, however,`mutate()` allows us to create and 
immediately use the variable (`year2`), something that `transform()` will not do.
...
in our harMet15.09.11 dataframe yet we could see the output. 
...
``` {r challenge-code-dplyr, echo=FALSE}
