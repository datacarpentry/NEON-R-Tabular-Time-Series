...
through understanding date-time classes and learning how to
convert character class data to a POSIX date-time class
...
converting the date field to an `R` `Date` data class.  In this lesson we will 
...
library to learn how to work with date-time data classes.
...
Let's revisit the data structure to start, looking specifically at the date-time 
field. Have a look at the `datetime` field in the `harMet_15Min` object. It is 
in which data class? 
...
Our `datetime` field is in a character data type. We need to convert it to 
...
We've lost the time stamp associated with the `datetime` field. The `as.Date` 
method worked great for a field with just dates, but it doesn't 
...
As we just saw, when we use the`as.Date` method to convert a date field that is
...
##Convert a Data Frame Field to a Date-time Class
...
following designations for the components of the date-time field:
...
Using the syntax we've learned, we can convert the entire `datetime` field into 
`POSIXct` data class.
...
We need to designate the correct time zone when converting the field to POSIX.
...
field in the object to an `POSIXct` class.
...
#view structure and time zone of the newly defined datetime field
