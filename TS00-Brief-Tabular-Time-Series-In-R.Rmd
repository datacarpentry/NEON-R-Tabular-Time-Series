---
layout: post
title: "Lesson 00: Brief Time Series in R- Simple Plots & Date Conversion"
date:   2015-10-25
...
stored in .csv format into R. It will explore column classes and will walk
...
But much ecological data are not suitable for factors. An example of strings as
...
Later on it is easy to turn specific fields into factors using `as.factor()`,
...
classes and not on others (e.g., a <- "mouse"-"sparrow"; subtracting two **chr** 
data points makes no sense).  Additionally, the attributes and properties of
different data classes varies (e.g., there is not point in showing the minimum
value for `SpeciesObserved` a **chr** type but there is for `prec` a **num**
type.) These and many other processes mean that assigning data classes makes for
...
air temperature as it changes over time. We have a date-time field, `date`, so 
...
Let's have a look at the data class of the fields that we are trying to plot.
...
value to an actual date-time class that can be displayed quantitatively. Lucky for 
us, `R` has a `date` class.  We can reassign the `harMetdaily$date` field using `as.Date`.

```{r convert-date-time }

#convert field to date class
...
much more quickly now that R recognizes the date as a date field. R can aggregate
...
1. Create a quick plot of the precipitation throughout the years of the study.
