...
the `datetime` field to a POSIXct class date-time data type. 
...
December 2011, so we set our criteria to be "any `datetime` that is greater than
...
with this data and found out that the missing values are given an NA value. One 
...
calculations
or using a function on fields that may contain NA values.  Missing values can
...
3) If needed, convert the data class of different fields.
